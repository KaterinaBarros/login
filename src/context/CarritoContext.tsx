...
import InstrumentoParams from '../componentes/ItemInstrumento';
...
export function CarritoContextProvider({ children }: { children: ReactNode }){
    ...
    const removeItemCarrito = async (product: Instrumento) => {
        ...
        //const platoIndice = cart.findIndex((objeto:Instrumento) => objeto.id === product.id)
    ...
    const calcularTotalCarrito = async () => {
        let total:number = 0;
        cart.forEach(async (element:Instrumento) => {
            total += element.precio * element.cantidadVendida;
        });
